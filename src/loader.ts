 ...
 */
export function installQueueStub(): void {
    if (typeof window === 'undefined') return;
    if (window.ScheduleME) return;
    const stub = function (this: unknown, ...args: unknown[]) {
        const fn = window.ScheduleME as ScheduleMEGlobal & { q?: unknown[] };
        fn.q = fn.q || [];
 ...
 */
export function ensureLoader(url: string): Promise<void> {
    ...
    const promise = new Promise<void>((resolve, reject) => {
        ...
        if (existingScript) {
            ...
            existingScript.addEventListener('error', () =>
                reject(new Error(`Failed to load ScheduleME loader from ${src}`)),
            );
            return;
        }
 ...
 */
export function getScheduleMEGlobal(): ScheduleMEGlobal | null {
